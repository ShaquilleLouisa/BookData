...
class BookData(toga.App):
    def startup(self):
        ...
        BookData.mainBox.add(StartScreen.startup(self, self.openBookInfoScreen, self.saveData))
    ...
    def openSettingsScreen(self, *_: any):
        BookData.closeScreen()
        BookData.mainBox.style.update(background_color=Widgets.primaryColor)
        BookData.mainBox.add(SettingsScreen.startup(self, self.openSettingsScreen, self.saveData))
